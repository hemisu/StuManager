...
if ( ! function_exists('thumb'))
{
	...
	if ( ! function_exists('pages'))
	{
		...
		function pages($num, $curr_page, $perpage = 20, $urlrule = '', $array = array(),$setpages = 10) {
			$setpages=10;
			$multipage = '<nav><ul class="pagination pagination-sm no-margin pull-right">';
			if($num > $perpage) {
				$page = $setpages+1;//
				$offset = ceil($setpages/2-1);//4
				$pages = ceil($num / $perpage);//40/20=2
				
				$from = $curr_page - $offset;//1-4=-3
				$to = $curr_page + $offset;//1+4=5
				$more = 0;
				if($page >= $pages) {//11>=2
					$from = 1;
					$to = $pages-1;//2-1=1
...
/*			$url_title="";
	...
	if(!function_exists('page_list_url'))
	{
		...
		{
			if($use_baseurl)
			return SITE_BASE. $base_url . '/{$page}' ;
			else
			return SITE_BASE. $base_url  ;
		}
	}
